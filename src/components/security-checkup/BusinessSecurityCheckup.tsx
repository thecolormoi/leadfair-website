...
// ─── Theme (purple/violet for security) ─────────────────
const accent = {
  from: '#8b5cf6',
  to: '#6366f1',
  primary: '#8b5cf6',
...
const inputStyle = 'w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-5 py-4 text-white text-lg placeholder-white/20 focus:border-[#8b5cf6]/50 focus:ring-2 focus:ring-[#8b5cf6]/10 focus:bg-white/[0.06] outline-none transition-all'
  ...
  return (
    <button
      ...
      className={`group w-full flex items-center gap-3.5 px-4 py-3.5 rounded-xl border transition-all duration-200 text-left ${
        selected
          ? 'border-[#8b5cf6]/40 bg-[#8b5cf6]/[0.08] shadow-[0_0_24px_rgba(139,92,246,0.08)]'
    ...
    >
      <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all duration-200 ${
        selected
          ? 'bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] text-white shadow-lg shadow-[#8b5cf6]/30'
  ...
  return (
    <div className="animate-slideIn">
      {isFirstInCategory && catInfo && (
        <div className="mb-5">
          <span className="inline-block text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-1 rounded-full bg-[#8b5cf6]/10 text-[#a78bfa] border border-[#8b5cf6]/20">
            {catInfo.name}
          </span>
        </div>
      )}
...
function LeadCapture({
  ...
  return (
    <div className="animate-slideIn text-center">
      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] flex items-center justify-center mx-auto mb-6 shadow-lg shadow-[#8b5cf6]/20">
        <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
      </div>
      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">Your Security Report Is Ready</h2>
    ...
    <div className="min-h-[calc(100dvh-64px)] flex flex-col items-center py-12 px-4">
      <div className="w-full max-w-2xl space-y-8 animate-slideIn">
        ...
        <div className={`${glass} p-8 sm:p-10 text-center`}>
          <p className="text-sm text-white/30 mb-4">Security Score for {businessName}</p>
        ...
        <div className={`${glass} p-6 sm:p-8`}>
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] flex items-center justify-center flex-shrink-0 shadow-lg shadow-[#8b5cf6]/20">
            ...
            <div>
              ...
              <p className="text-xs text-white/30">AI-generated risk analysis based on your specific answers</p>
            </div>
          </div>
          {reportLoading ? (
            <div className="flex items-center gap-3 py-12 justify-center">
              <div className="w-5 h-5 border-2 border-[#8b5cf6] border-t-transparent rounded-full animate-spin" />
              <p className="text-sm text-white/40">{loadingStatus || 'Analyzing your security posture...'}</p>
            </div>
          ) : aiReport ? (
  ...
  async function handleLeadSubmit(info: { name: string; email: string; phone: string }) {
    ...
    try {
      localStorage.setItem('leadfair_audit', JSON.stringify({
        type: 'security', businessName: String(answers['business-name'] || ''),
    ...
    try {
      ...
      if (reportText) formData.append('security-report', reportText)
    ...
    return (
      <div className="min-h-[calc(100dvh-64px)] flex items-center justify-center px-4">
        <div className="text-center max-w-lg animate-slideIn">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] flex items-center justify-center mx-auto mb-8 shadow-lg shadow-[#8b5cf6]/20">
            <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">Security Checkup</h1>
          <p className="text-white/40 mb-6 text-base leading-relaxed max-w-md mx-auto">
            Find out how protected your business really is. Get a personalized risk assessment with specific recommendations.
          </p>
      ...
      {phase === 'questions' && (
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-white/[0.04]">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#8b5cf6] to-[#6366f1] flex items-center justify-center">
              <svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
              </svg>
            </div>
            <span className="text-sm font-semibold text-white/70">Security Checkup</span>
          </div>
                ...
                <div className="flex items-center justify-between px-7 sm:px-9 py-4 border-t border-white/[0.04]">
                  ...
                  <button
                    ...
                    className={`text-sm font-semibold px-6 py-2.5 rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
                      canProceed()
                        ? 'bg-gradient-to-r from-[#8b5cf6] to-[#6366f1] text-white hover:shadow-lg hover:shadow-[#8b5cf6]/20'
